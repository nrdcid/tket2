...
fn circ_flat_quantum() -> Hugr {
    fn build() -> Result<Hugr, BuildError> {
        ...
        h.finish_hugr_with_outputs(wires)
    }
    build().unwrap()
}

/// A circuit with some (unsupported) borrow array operations.
fn circ_borrow_array() -> Hugr {
    fn build() -> Result<Hugr, BuildError> {
        ...
        h.finish_hugr_with_outputs([arr])
    }
    build().unwrap()
}
...
fn test_pytket_pass(
    ...
    let mut new_hugr = hugr.clone();
    let updated_regions = encoded.reassemble_inplace(&mut new_hugr, None).unwrap();

    let quantum_ops = updated_regions
        .iter()
        .map(|region| count_quantum_gates(&new_hugr, *region))
        .sum::<usize>();
    assert_eq!(quantum_ops, num_remaining_gates);
}

/// Helper method to count the number of quantum operations in a hugr region.
fn count_quantum_gates(hugr: &Hugr, region: Node) -> usize {
